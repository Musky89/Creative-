...
function pickPath(args: {
  ...
  if (t.productionMode === "IDENTITY") {
    reasons.push(
      "IDENTITY: abstract mark-study tiles via flux-general — route labels and strategy are platform-composed.",
    );
    return { pathId: "fal-ai/flux-general", kind: "TEXT_TO_IMAGE" };
  }

  if (t.productionMode === "ECOMMERCE_FASHION") {
    reasons.push("ECOMMERCE_FASHION: garment and scene truth — default flux-general.");
    if (t.targetType === "LIFESTYLE_SCENE" && hasStyle) {
      reasons.push("Lookbook/lifestyle + style ref — LoRA text-to-image for DNA lock.");
      return { pathId: "fal-ai/flux-lora", kind: "LORA_TEXT_TO_IMAGE" };
    }
    return { pathId: "fal-ai/flux-general", kind: "TEXT_TO_IMAGE" };
  }
  ...
  if (t.productionMode === "EXPORT_PRESENTATION") {
    if (t.targetType === "LIFESTYLE_SCENE") {
      reasons.push(
        "EXPORT_PRESENTATION: optional mood plate — flux-general; slide copy and layout stay platform-owned.",
      );
      return { pathId: "fal-ai/flux-general", kind: "TEXT_TO_IMAGE" };
    }
    reasons.push("EXPORT_PRESENTATION: composition-only for non-raster deck targets.");
