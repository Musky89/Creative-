...
import { fashionBatchCount } from "./mode-identity-fashion-export";
...
> {
  ...
  if (input.mode === "IDENTITY") {
    heroAssetIntent = `IDENTITY exploration — neutral board field or abstract mark study for "${input.selectedConcept.conceptName}"; no final trademark in raster; ${input.visualDirection.slice(0, 120)}`;
    secondaryAssetIntent =
      "Parallel route panels: symbol / wordmark / combination exploration tiles — composer labels and strategy ribbon.";
    compositionIntent =
      "Board-style grid: strategy summary band → three equal route cells → application strip; platform typography for all route names.";
    negativeSpaceIntent =
      "Generous gutters between route tiles; consistent neutral ground for comparison.";
    realismBias = "illustrative";
    typographyIntent =
      "IDENTITY: all route names, strategy summary, and captions are platform vector — FAL outputs are mark studies only.";
    exportTargets = [
      "identity-board-png",
      "route-exploration-deck",
      "layer-manifest",
      ...cfg.exportFormats,
    ];
    reviewFocus = [
      "route differentiation at a glance",
      "board consistency (scale, lighting convention)",
      "no final logo lockup baked only in FAL",
      "strategy-to-form narrative visible",
    ];
    modeConstraints = [
      "IDENTITY: presentation boards, not ad frames",
      "IDENTITY: three-route comparison minimum in compose",
      ...modeConstraints,
    ];
  }

  if (input.mode === "ECOMMERCE_FASHION") {
    const fn = fashionBatchCount(input.fashionBatchPreset);
    heroAssetIntent = `Fashion production set (${fn} output${fn > 1 ? "s" : ""}): ${input.selectedConcept.conceptName} — model/garment/scene per shot plan; ${input.visualDirection.slice(0, 130)}`;
    secondaryAssetIntent =
      "Detail macro + alternate family (ecom vs editorial vs social vs outfit story) per batch rules.";
    compositionIntent =
      "Composer stacks headline/CTA per output family; FAL delivers garment truth rasters only.";
    negativeSpaceIntent =
      "Safe crop for PDP and 4:5 social; editorial frames may use full bleed with type safe zone.";
    realismBias = "photoreal";
    typographyIntent =
      "Fashion: product truth in image; promo type in compose — family-specific headline/CTA from variant table.";
    exportTargets = [
      `fashion-batch-${fn}`,
      "pdp-hero",
      "lookbook-frame",
    ...
    reviewFocus = [
      "garment/fabric truth",
      "catalog vs editorial bias respected",
      "family coherence across batch",
      "SKU-consistent silhouette",
    ];
    modeConstraints = [
      "ECOMMERCE_FASHION: model role supports garment read",
      input.visualSpecNotes?.toLowerCase().includes("editorial")
        ? "Bias: editorial lookbook frames allowed"
        : "Bias: catalog-first e-com clarity",
      ...modeConstraints,
    ];
  }
  ...
  if (input.mode === "EXPORT_PRESENTATION") {
    heroAssetIntent =
      "EXPORT: optional mood plate or key visual slot — not required; deck is composer-first.";
    secondaryAssetIntent =
      "Supporting visual strip for selected concept; primary narrative is text blocks.";
    compositionIntent =
      "16:9 slide: title band → hero or split visual → body copy → footer; grouped rationale per slide index.";
    negativeSpaceIntent =
      "Margins for projector + PDF; no dense wall of text in single rect.";
    realismBias = "mixed";
    typographyIntent =
      "EXPORT: deck typography hierarchy — title, subtitle, body; campaign story arc across slides.";
    exportTargets = [
      "client-deck-png-slides",
      "deck-manifest-json",
      "pdf-handoff-stub",
      ...cfg.exportFormats,
    ];
    reviewFocus = [
      "story arc order",
      "rationale grouping clarity",
      "concept + headline + CTA surfaced cleanly",
      "client-facing tone",
    ];
    modeConstraints = [
      "EXPORT_PRESENTATION: client-facing assembly",
      "Slides index 0–5 for default deck sections",
      ...modeConstraints,
    ];
  }
        ...
        : input.mode === "SOCIAL"
          ...
          : input.mode === "PACKAGING"
            ...
            : input.mode === "RETAIL_POS"
              ...
              : input.mode === "IDENTITY"
                ? "IDENTITY: print/PDF board export; neutral ICC-safe backgrounds."
                : input.mode === "ECOMMERCE_FASHION"
                  ? "FASHION: web-optimized PDP + high-res lookbook master where needed."
                  : input.mode === "EXPORT_PRESENTATION"
                    ? "EXPORT: slide masters for PDF; optional 16:9 PNG sequence."
