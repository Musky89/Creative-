...
export function evaluateProductionOutput(
  ...
  if (input.mode === "IDENTITY" && plan) {
    checklist.push({
      id: "identity-three-routes",
      label: "IDENTITY: three route targets in visual bundle",
      ok: true,
      note: "Compose preview maps hero/secondary/tertiary URLs to routes A/B/C.",
    });
    modeReviewSummary.push(
      "IDENTITY: route distinction — three FAL mark studies vs one hero.",
      "IDENTITY: final lockups are vector — rasters are exploration-only.",
      "IDENTITY: strategy and captions are platform-composed on the board.",
    );
  }

  if (input.mode === "ECOMMERCE_FASHION" && plan) {
    const n = fashionBatchCount(input.fashionBatchPreset);
    checklist.push({
      id: "fashion-batch",
      label: `ECOMMERCE_FASHION batch (${n} slot(s))`,
      ok: n >= 1,
    });
    modeReviewSummary.push(
      "ECOMMERCE_FASHION: garment truth — photoreal targets; LoRA optional on lifestyle + style ref.",
      "ECOMMERCE_FASHION: family rotation (clean e-com, editorial, product social, outfit story).",
      "ECOMMERCE_FASHION: headline/CTA and optional detail inset are composed, not baked in FAL.",
    );
  }

  if (input.mode === "EXPORT_PRESENTATION" && plan) {
    checklist.push({
      id: "export-deck-first",
      label: "EXPORT_PRESENTATION: structured deck sections",
      ok: true,
      note: "Use exportSlideIndex in the test UI to page the compose preview across slides.",
    });
    modeReviewSummary.push(
      "EXPORT_PRESENTATION: slide master — title, body, footer from structured sections.",
      "EXPORT_PRESENTATION: optional mood plate via LIFESTYLE_SCENE + flux-general.",
      "EXPORT_PRESENTATION: client-facing assembly without ad-frame logic.",
    );
  }
