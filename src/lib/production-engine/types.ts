...
export type ProductionEngineInput = {
  ...
  /** IDENTITY: third route tile; ECOMMERCE_FASHION: optional detail inset URL. */
  ...
  /** ECOMMERCE_FASHION: batch over output families (default 1). */
  fashionBatchPreset?: FashionBatchPreset;
  /** ECOMMERCE_FASHION: subset of families to cycle. */
  fashionOutputFamilies?: FashionOutputFamily[];
  /** ECOMMERCE_FASHION: compose preview index (default 0). */
  fashionVariantIndex?: number;
  /** EXPORT_PRESENTATION: which deck section to render in compose preview (0–5). */
  exportSlideIndex?: number;
  /** IDENTITY: optional route panel to emphasize in compose (A|B|C). */
...
export type ProductionEngineRunResult = {
  ...
  /** ECOMMERCE_FASHION: per-slot copy + shot notes. */
  fashionVariants?: FashionVariantCopy[];
  /** EXPORT_PRESENTATION: deck section content for UI / compose. */
  exportDeckSections?: ExportDeckSection[];
  jobs: ProductionJob[];
