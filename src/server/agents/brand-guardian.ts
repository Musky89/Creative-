...
> = {
  ...
  buildSystemPrompt: () =>
    [
      ...
      "Keys include: scoreSummary, verdict, issues, recommendations, narrativeCoherence, toneCoherence, visualCoherence, campaignCoreAlignmentNotes, frameworkAssessment, frameworkExecution, qualityVerdict, distinctivenessAssessment, brandAlignmentAssessment, toneAlignment, languageCompliance, bannedPhraseViolations, toneDistinctiveness, rhythmCompliance, signatureDeviceUsage, culturalAlignment, regenerationRecommended, regenerationReasons,",
      ...
      "**Campaign Core audit:** If STRATEGY includes `campaignCore`, judge whether CONCEPT (winner), COPY, and VISUAL_SPEC feel like **one campaign** expressing that spine. Set narrativeCoherence / toneCoherence / visualCoherence to ALIGNED | MIXED | DRIFT. DRIFT if any stage introduces a second big idea, wrong tension, or unrelated visual world. campaignCoreAlignmentNotes: min ~40 chars citing evidence.",
  ...
  buildUserPrompt: (formattedContext, options) =>
    [
      ...
      "",
      "Compare COPY, CONCEPT, and VISUAL_SPEC to **campaignCore** (if in STRATEGY) first, then Brand Bible and **Brand Creative DNA** (taste engine + voice/rhythm/devices/codes/tension).",
